...
%        los_model = slip2insar_okada(xin,yin,losl,zel,znl,zul,slip_model_vs);
...
lat_eq = 19.61;
[xo,yo] = ll2xy(lon_eq,lat_eq,ref_lon);
[x1,y1,losin]=grdread2([this_track,'/','rng_offset_ll','.grd']);   % in the unit of cm
Nmin = 2;
Nmax = 150;
[~,~,zout,~,~,~,~,~,~]=make_insar_downsample(x1,y1,losin,this_npt,Nmin,Nmax,'mean'); % same with downsample
fprintf("Npt: %d \n",length(zout));

[x1,y1,losin]=grdread2([this_track,'/','los_ll_low','.grd']);   % in the unit of cm

[xout,yout,zout,~,rms_out,xx1,xx2,yy1,yy2]=make_insar_downsample(x1,y1,losin./100,this_npt,Nmin,Nmax,'mean'); % same with downsample
fprintf("Npt: %d \n",length(zout));
...
Ngood = length(zdata);    % number of valid data
r_good = Ngood / n_block;  % Proportion of valid data
...
if (Ngood > 0)
    ...
    % number of input x y ,unique may because the non-value
    lx = length(unique(x));
    ly = length(unique(y));
    %   lx=length(x);
    %   ly=length(y);
